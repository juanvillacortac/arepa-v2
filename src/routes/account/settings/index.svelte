...
<div class="flex flex-col mx-auto space-y-4 w-full p-4 lg:w-[90%]">
  <div class="flex font-bold space-x-2 text-xs text-gray-400 uppercase">
    ...
    <p>Account</p>
    <span>/</span>
    <a href="/account/settings" class="hover:underline">Settings</a>
  </div>
  <h3 class="font-bold font-title text-black text-3xl dark:text-white">
    Account Settings
  </h3>
  <form
    class="border rounded-lg flex flex-col mx-auto space-y-4 border-gray-300 w-full p-4 items-center lg:w-3/10 dark:border-dark-400"
    on:submit|preventDefault={submitCustomer}
  >
    {#if c}
      <div
        class="bg-gradient-to-br border rounded-full flex font-bold font-title from-green-300 to-pink-600 border-gray-200 h-72px text-white text-4xl leading-[0] w-72px items-center justify-center uppercase dark:bg-dark-600 dark:from-green-400 dark:to-pink-700"
      >
    ...
    <button
      class="rounded flex font-bold bg-dark-800 text-center text-sm w-full py-2 px-6 transform-gpu duration-200 justify-center items-center !text-white hover:shadow disabled:cursor-not-allowed disabled:opacity-70 not-disabled:hover:-translate-y-px"
      disabled={loading || !c}
      type="submit"
