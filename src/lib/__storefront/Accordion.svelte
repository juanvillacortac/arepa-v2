...
<div class="rounded flex-col flex w-full overflow-hidden">
  <button
    ...
    class="flex font-bold font-title bg-dark-800 shadow-sm text-white w-full p-2 z-20 justify-between items-center"
    on:click={() => (open = !open)}
    use:tooltip
  >
  ...
  {#if open}
    <div class="bg-gray-100 p-2 dark:bg-dark-700" transition:slide>
      {@html html || ''}
    </div>
  {/if}
</div>
