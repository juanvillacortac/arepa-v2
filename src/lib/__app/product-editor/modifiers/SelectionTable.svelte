    ...
    <tbody class="z-10 relative">
      {#each $items?.filter((i) => i.active) as i, idx (i.internalId)}
        ...
        >
          ...
          <td class="py-4 px-6">
            <input
              class="bg-white border rounded border-gray-300 text-xs text-right leading-tight py-2 px-3 w-18 appearance-none dark:bg-dark-700 dark:border-dark-400 focus:outline-none focus:shadow-outline"
              type="number"
              min={0}
              step="any"
              required
