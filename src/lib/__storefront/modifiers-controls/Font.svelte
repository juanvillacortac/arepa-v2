...
{:else}
  <div class="w-full grid gap-4 grid-cols-3 lg:w-full  lg:grid-cols-3">
    <button
      class="border-dashed rounded flex border-2 text-lg w-full p-2 transform transition-transform text-gray-200 duration-200 items-center justify-center dark:border-dark-400 dark:text-gray-600"
      title="Unset font"
      on:click={() => (value = {})}
      use:tooltip
    ...
    {#each items || [] as i}
      <button
        class="rounded border-2 text-lg w-full p-1 transform transition-transform duration-200 dark:border-dark-400"
        title={i.name}
        on:click={() =>
          (value = {
            itemId: i.id,
